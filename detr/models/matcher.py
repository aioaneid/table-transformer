...
from util.box_ops import (
    box_cxcywh_to_xyxy,
    box_cxcywh_to_xyxy_with_bounds,
    generalized_box_iou,
    cross_bounded_box_iou_with_bounds,
)
...
def last_step(cs):
    # print("len(cs): {}".format(len(cs)))
    # result = [None] * len(cs)
    # for i, c in enumerate(cs):
    #     result[i] = robust_linear_sum_assignment(c)
    #     print("i: {} c: {} lsa: {}".format(i, c, result[i]))
    # return result
    # https://github.com/scipy/scipy/blob/main/scipy/optimize/rectangular_lsap/rectangular_lsap.cpp
    return [robust_linear_sum_assignment(c) for c in cs]
        ...
        tuple(
            zip(
                *[
                    (e[1][1], e[0])
                    for e in sorted(
                        enumerate(list(zip(*robust_linear_sum_assignment(c)))),
                        key=lambda e: e[1][1],
                    )
                ]
            )
...
def _core_original_hungarian_matching(
    outputs,
    targets,
    cost_bbox_factor,
    cost_class_factor,
    ...
    cost_bbox = (
        box_ops.cross_lp_loss_with_bounds_cxcywh if enable_bounds else torch.cdist
    )(
    ...
    tgt_bbox_xyxy = (
        box_cxcywh_to_xyxy_with_bounds(tgt_bbox)
        if enable_bounds
        else box_cxcywh_to_xyxy(tgt_bbox)
    )
    giou = (
        cross_bounded_box_iou_with_bounds if enable_bounds else generalized_box_iou
    )(
...
def _original_hungarian_matching(
    outputs,
    targets,
    cost_bbox_factor,
    cost_class_factor,
    ...
    return last_step(
        _core_original_hungarian_matching(
            outputs,
            targets,
            cost_bbox_factor,
            cost_class_factor,
...
def _final_hungarian_matching_params(outputs, targets, enable_bounds):
    ...
    return cost_bbox, giou, prob_class
...
def _core_final_hungarian_matching(
    ...
    cost_bbox, giou, prob_class = _final_hungarian_matching_params(
        outputs, targets, enable_bounds
    )
    C = (
        cost_bbox_factor * cost_bbox
        - cost_giou_factor * giou
        - cost_class_factor * prob_class
    )
    c = C.cpu()
    # print("c: {}".format(c))
    return [c[i, :, : len(v["labels"])] for i, v in enumerate(targets)]
...
def _core_compat_hungarian_matching(
    outputs,
    targets,
    cost_bbox_factor,
    cost_class_factor,
...
def _compat_hungarian_matching(
    outputs,
    targets,
    cost_bbox_factor,
    cost_class_factor,
    ...
    return last_step(
        _core_compat_hungarian_matching(
            outputs,
            targets,
            cost_bbox_factor,
            cost_class_factor,
...
def _core_inplace_hungarian_matching(
    ...
    out_bbox = outputs["pred_boxes"]  # [batch_size, num_queries, 4]
    C = torch.cdist(out_bbox, tgt_bbox, p=1)  # [batch_size, num_queries, max_size]
    C *= cost_bbox_factor
    out_bbox_xyxy = box_cxcywh_to_xyxy(out_bbox)
    tgt_bbox_xyxy = box_cxcywh_to_xyxy(tgt_bbox)
    giou = generalized_box_iou(
...
def _core_tr_hungarian_matching(
    ...
    out_bbox = outputs[
        "pred_boxes"
    ]  # [batch_size, num_queries, 8 if enable_bounds else 4]
    cost_bbox = (
        box_ops.cross_lp_loss_with_bounds_cxcywh if enable_bounds else torch.cdist
    )(
        out_bbox, tgt_bbox, p=1
...
class HungarianMatcher(nn.Module):
    ...
    def __init__(
        ...
        algo_name = os.environ.get("HUNGARIAN_MATCH_ALGO")
        match algo_name:
            case "tr":
            ...
            case "compat":
                self.algo = _compat_hungarian_matching
            case "inplace":
                self.algo = _inplace_hungarian_matching
            case "new":
                self.algo = _new_hungarian_matching
            case "original":
                self.algo = _original_hungarian_matching
            case "test":
                self.algo = _test_hungarian_matching
            case _:
                self.algo = _compat_hungarian_matching
        print("Matcher algo: {}".format(self.algo))
