...
class RandomSizeCrop(object):
    ...
    def __call__(self, img: PIL.Image.Image, target: dict):
        w = torch.randint(self.min_size, min(img.width, self.max_size) + 1, ()).item()
        h = torch.randint(self.min_size, min(img.height, self.max_size) + 1, ()).item()
...
class RandomResize(object):
    ...
    def __call__(self, img, target=None):
        size = self.sizes[torch.randint(0, len(self.sizes), ()).item()]
...
class RandomPad(object):
    ...
    def __call__(self, img, target):
        pad_x = torch.randint(0, self.max_pad + 1, ()).item()
        pad_y = torch.randint(0, self.max_pad + 1, ()).item()
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        ...
        if "boxes" in target and len(target["boxes"]) > 0:
            boxes = target["boxes"]
            # for box in boxes:
            #     assert box_ops.is_present(box).all(), box
