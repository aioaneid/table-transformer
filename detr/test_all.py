...
def make_present(c):
    return torch.cat([c[..., :2].min(c[..., 2:]), c[..., :2].max(c[..., 2:])], dim=-1)
...
class Tester(unittest.TestCase):
    ...
    def test_generalized_box_iou_multiple_dimensions(self):
        ...
        assert_close(
            box_ops.generalized_box_iou(a[None, None, None, :], b[None, None, None, :]),
            torch.Tensor([[[[ab]]]]),
            rtol=0,
            atol=0,
        )
        assert_close(
            box_ops.generalized_box_iou(torch.stack([a, a, b, b]), torch.stack([a, b])),
...
class CrossBoundedBoxIouTester(unittest.TestCase):
    ...
    def test_different_dimensions_tight(self):
        a = torch.tensor([1, 1, 2, 2])
        b = torch.tensor([1, 2, 3, 5])
        aa = torch.cat((a, a))
        bb = torch.cat((b, b))
        ab = -0.1250
        pred = torch.stack([a, a, b]).repeat(5, 1, 1)
        target = torch.stack([aa, bb])[None, ...].expand(5, 2, 8)
        assert_close(
            box_ops.cross_bounded_box_iou_with_bounds(
                pred, target
            ),
            torch.Tensor([
                [1, ab], [1, ab], [ab, 1]]).repeat(5, 1, 1),
            rtol=0,
            atol=0,
        )
    ...
    def test_empty_prediction_outside_outer_boundary(self):
        ...
        assert_close(
            box_ops.cross_bounded_box_iou_with_bounds(c[None, :], bb[None, :]),
            torch.Tensor([[1 / 2**2 / 2 + 1 / 2]]),
            rtol=0,
            atol=0,
        )

    def test_empty_prediction_outside_outer_boundary2(self):
        o = torch.tensor([0, 0, 1, 1])
        h = torch.tensor([1, 2, 0, 5])
        b = torch.full((4,), 2 + 1 / 64)
        ho = torch.cat((h, o))
        assert_close(
            box_ops.cross_bounded_box_iou_with_bounds(b[None, :], ho[None, :]),
            torch.Tensor([[1 / (2 + 1 / 64) ** 2 / 2 + 1 / 2]]),
            rtol=0,
            atol=0,
        )
    ...
    def test_empty_prediction_outside_inner_boundary(self):
        o = torch.tensor([1, 2, 0, 5])
        h = torch.tensor([0, 0, 1, 1])
        b = torch.full((4,), 2)
        ho = torch.cat((h, o))
        assert_close(
            box_ops.cross_bounded_box_iou_with_bounds(b[None, :], ho[None, :]),
            torch.Tensor([[-1 + 1 / 2 + 1 / 4 / 2]]),
            rtol=0,
            atol=0,
        )
    ...
    def test_random_tight(self):
        ...
        cc = torch.cat((c, c), dim=-1)
        assert_close(
            box_ops.cross_bounded_box_iou_with_bounds(b, cc),
            box_ops.generalized_box_iou(b, c),
        )
...
class CrossDistTester(unittest.TestCase):
    ...
    def test_empty_prediction_outside_inner_boundary(self):
        ...
        ho = torch.cat((h, o))
        assert_close(
            _cross_dist_with_bounds(b[None, :], ho[None, :], p=1),
            torch.Tensor([[4]]),
            rtol=0,
            atol=0,
        )
