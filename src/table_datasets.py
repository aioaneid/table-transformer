...
#     left = max(1, int(round(crop_bbox[0] - max_margin * torch.rand(()).item())))
#     top = max(1, int(round(crop_bbox[1] - max_margin * torch.rand(()).item())))
#     right = min(width, int(round(crop_bbox[2] + max_margin * torch.rand(()).item())))
#     bottom = min(height, int(round(crop_bbox[3] + max_margin * torch.rand(()).item())))
...
class PDFTablesDataset(torch.utils.data.Dataset):
    def __init__(self, root, transforms, max_size=None, do_crop=True, make_coco=False,
        ...
        if max_size is not None and max_size < len(self.page_ids):
            ...
            indices = torch.randperm(len(self.page_ids))[:max_size].sort().values
    ...
    def __getitem__(self, idx):
        ...
        if self.debug_dataset:
            print("page_id: {} img_tensor.shape: {}".format(page_id, img_tensor.shape))
