...
def get_args():
    ...
    parser.add_argument("--epoch_seeds", type=int, nargs="*")
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Print the page IDs as they are generted from PDFTablesDataset.",
    )
    ...
    if args.mode == "train":
        train_datasets = [
            PDFTablesDataset(
                ...
                or "{}_filelist.txt".format(args.train_split_name),
                class_map=class_map,
...
def train(args, model, criterion, postprocessors, device):
    ...
    if args.model_load_path:
        ...
        if not args.load_weights_only and "rng_state" in checkpoint:
            rng_state_dict = checkpoint["rng_state"]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, (dataset_val, data_loader_val) in enumerate(dataset_val_loaders):
            stats, _ = evaluate(
                ...
                device,
                None,
                "{}_{}_epoch{}_valds{}".format(
                    args.coco_eval_prefix, model_path_stem, epoch, i
                )
        ...
        torch.save(
            {
                ...
                "rng_state": get_rng_state_dict(device),
            },
            model_save_path,
        )
