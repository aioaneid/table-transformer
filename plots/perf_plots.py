...
def get_args_parser():
    ...
    parser.add_argument("--path_regex", type=str)
...
def draw_plots():
    ...
    print(args)
    filename_pattern = re.compile(args.file_regex) if args.file_regex else None
    path_pattern = re.compile(args.path_regex) if args.path_regex else None
    print(
        "filename_pattern: {} path_pattern: {}".format(filename_pattern, path_pattern)
    )
    ...
    for filepath in args.input_logs:
        print(filepath)
        filename_match = (
            filename_pattern.match(filepath.name)
            if filename_pattern
            else path_pattern.match(str(filepath))
        )
        gi = (filename_pattern or path_pattern).groupindex
        has_fraction_group = "fraction" in gi
        has_all_object_segmented_group = "all_objects_segmented" in gi
        has_kept_group = "kept" in gi
        if has_fraction_group:
            fraction_group = (
                filename_match.group("fraction") if filename_match else None
            )
            fraction = fraction_group or "unknown"
        ...
        values = None
        latest_epoch = epoch_from_filename
        file_stats = {}
        with open(filepath, mode="rt") as f:
            for line in f:
                ...
                if (train_len_match or True) and not has_fraction_group:
        ...
        for epoch, values in file_stats.items():
            ...
            v.extend([(filepath, None)] * (epoch - len(v)))
    ...
    for key in ("ap50", "ap75", "ap", "ar"):
        ...
        ax.tick_params(right=True, labelleft=False, labelright=True)
        ax.tick_params(right=True, labelleft=False, labelright=True, which="minor")
        ...
        for index, (fraction, v) in enumerate(d.items()):
            x = [i + 1 for i in range(epochs) if i < len(v) and v[i][1] is not None]
